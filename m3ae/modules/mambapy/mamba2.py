...
from typing import Iterable, NamedTuple, cast, Union, Tuple
...
class Mamba2(nn.Module):
    ...
    def forward(self, u: Tensor, h: Union[InferenceCache, None] = None):
        ...
        # 验证输入维度是否为chunk_size的整数倍，如果不是则进行线性变换
        # u_ori_shape = u.shape[1]
        # if u.shape[1] % self.args.chunk_size != 0:
        #     u = F.adaptive_avg_pool1d(u.permute(0, 2, 1), output_size=608).permute(0, 2, 1)

        if h:
            return self.step(u, h)
        # 插入检查和修改逻辑
        is_bad = torch.logical_or(torch.abs(self.A_log) > 10, torch.isnan(self.A_log))
        A_log_safe = torch.where(is_bad, torch.tensor(0.0, device=self.A_log.device), self.A_log)

        # self.A_log.data = torch.where(torch.isnan(self.A_log), torch.tensor(0.0, device=self.A_log.device), self.A_log)
        A = -torch.exp(A_log_safe)  # (nheads,)
        ...
        # if u_ori_shape != y.shape[1]:
        #     y = F.adaptive_avg_pool1d(y.permute(0, 2, 1), output_size=u_ori_shape).permute(0, 2, 1)
...
def segsum(x: Tensor, device: Device = None) -> Tensor:
    ...
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=device), diagonal=-1).to(x.device)
    x = x.masked_fill(~mask, 0).to(x.device)
    x_segsum = torch.cumsum(x, dim=-2)
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=device), diagonal=0).to(x.device)
...
#     mamba2block = Mamba2(Mamba2Config(d_model=768))
#
#     x = torch.randn(1,577,768)
#     y, h = mamba2block(x)
#     print(y.shape)
#     print(y)