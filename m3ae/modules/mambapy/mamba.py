...
class MambaBlock(nn.Module):
    ...
    def forward(self, x_v, x_q):
        ...
        # 两模态融合
        z_q = self.downsample(z_q, L_v)
        qv = z_v * z_q
        qv = F.silu(qv)

        # 融合信息交互
        y_q = self.downsample(y_q, L_v)
        y_v = y_v * qv
        y_q = y_q * qv
        y_q = self.upsample(y_q, L_q)
        # # 单模态融合
        # yz_v = y_v * z_v
        # yz_q = y_q * z_q
        #
        ...
        # # z_v = z_v.transpose(1, 2)
        # z_v = self.upsample(z_v, L_q)
        # yz_qv = y_q * z_v
        #
        # y_v = yz_v * yz_vq
        # y_q = yz_q * yz_qv

        out_v = self.out_proj(y_v)  # (B, L, D)
    ...
    def upsample(self, x, output_size):
        return F.interpolate(x.permute(0, 2, 1), size=output_size, mode='linear', align_corners=True).permute(0, 2, 1)
