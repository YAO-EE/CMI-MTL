...
class Pooler(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        self.dense = nn.Linear(hidden_size, hidden_size)  # 全连接层，维度都为hidden_size
        self.activation = nn.Tanh()  # 激活函数为tanh

    def forward(self, hidden_states):
        first_token_tensor = hidden_states[:, 0]  # 第一个token通常代表整个序列的语义信息
        pooled_output = self.dense(first_token_tensor)  # 对first_token_tensor进行全连接的线性变换
        pooled_output = self.activation(pooled_output)  # 用激活函数对线性变换结果进行激活处理
